...
int MAG_declination(MAGtype_Ellipsoid Ellip, MAGtype_CoordSpherical CoordSpherical, MAGtype_CoordGeodetic CoordGeodetic,
        MAGtype_MagneticModel *TimedMagneticModel, double *declination)
/*
The main subroutine that calls a sequence of WMM sub-functions to calculate the magnetic declination for a single point.
The function expects the model coefficients and point coordinates as input and returns the declination in `declination`. 
...
CALLS:  	MAG_AllocateLegendreFunctionMemory(NumTerms);  ( For storing the ALF functions )
                     ...
                     MAG_SecVarSummation(LegendreFunction, TimedMagneticModel, SphVariables, CoordSpherical, &MagneticResultsSphVar); Sum the Secular Variation Coefficients
                     MAG_RotateMagneticVector(CoordSpherical, CoordGeodetic, MagneticResultsSph, &MagneticResultsGeo); Map the computed Magnetic fields to Geodetic coordinates
                     MAG_CalculateGeoMagneticDeclinationElement(&MagneticResultsGeo, declination);   Calculate the Geomagnetic elements
                     NOT DONE: MAG_CalculateSecularVariationElements(MagneticResultsGeoVar, GeoMagneticElements); Calculate the secular variation of each of the Geomagnetic elements
 ...
 */
{
    ...
    MAG_RotateMagneticVector(CoordSpherical, CoordGeodetic, MagneticResultsSph, &MagneticResultsGeo); /* Map the computed Magnetic fields to Geodeitic coordinates  */
    MAG_RotateMagneticVector(CoordSpherical, CoordGeodetic, MagneticResultsSphVar, &MagneticResultsGeoVar); /* Map the secular variation field components to Geodetic coordinates*/
    *declination = MAG_CalculateGeoMagneticDeclinationElement(&MagneticResultsGeo);
    /* MAG_CalculateSecularVariationElements(MagneticResultsGeoVar, GeoMagneticElements);*/ /*Calculate the secular variation of each of the Geomagnetic elements*/
...
double MAG_CalculateGeoMagneticDeclinationElement(MAGtype_MagneticResults *MagneticResultsGeo)

/* Calculate just the declination from X,Y and Z components
INPUT     MagneticResultsGeo   Pointer to data structure with the following elements
                        double Bx;    ( North )
                        double By;	  ( East )
                        double Bz;    ( Down )
 ...
 */
{
    return RAD2DEG(atan2(MagneticResultsGeo->By, MagneticResultsGeo->Bx));
}
 ...
 */
{
    ...
    { /* Geoid correction required */
      /* To ensure that latitude is less than 90 call MAG_EquivalentLatLon() */
        ...
        DeltaHeight = 0; // will not get set if MAG_GetGeoidHeight has an error. We calculate height anyway and then return the error code from this function.
